...
def parse_email_transaction(subject: str, sender: str = "", body: str = "") -> dict:
    ...
    try:
        ...
        prompt = (
            ...
            f"Isi Email:\n{body[:7000]}\n\n"
            f"TUGAS KETAT:\n"
            f"1. PENTING UNTUK RESI/STRUK (Grab, Gojek, Shopee, Tokopedia, KAI, PLN, Mandiri, BNI, BCA):\n"
            f"   Jika email ini merupakan resi/struk/bukti pembayaran/notifikasi transfer yang memuat rincian tarif/harga/total bayar, EMAIL INI ADALAH TRANSAKSI FINANCIAL VALID! Set 'is_valid_transaction': true.\n"
            f"2. 'amount': Ekstrak nominal total transaksi dalam Rupiah (misal 'Rp 14.000' -> 14000.0, 'Rp 16.000' -> 16000.0). JANGAN mengembalikan 0 jika ada nominal rupiah pembayaran di email!\n"
            f"3. Jika email ini HANYALAH promosi, iklan, LinkedIn notification, Jobstreet, newsletter, atau konfirmasi tanpa nominal pembayaran, set 'is_valid_transaction': false dan amount: 0.\n"
            f"4. 'merchant': Nama penyedia/toko/layanan (contoh: 'GrabBike', 'GrabFood', 'Shopee', 'Indomaret', 'Gojek', 'Mandiri Livin', 'BNI Mobile', 'Access By KAI').\n"
            f"5. 'category': Pilih salah satu dari: 'Makanan', 'Transportasi', 'Kebutuhan Bulanan', 'Kebutuhan Bayi', 'Sosial & Ibadah', 'Hiburan', 'Liburan & Perjalanan', 'Perlengkapan & Acara', 'Keluarga', 'Transfer', 'Pendapatan', atau 'Lain-lain'.\n"
            f"6. 'payment_source': Tebak sumber pembayaran (misal 'Mandiri', 'BNI', 'Gopay', 'OVO', 'ShopeePay', 'Cash').\n"
            f"7. 'date': Tanggal transaksi YYYY-MM-DD. Jika tanggal di email dari tahun lalu atau tidak jelas, gunakan tanggal hari ini ({datetime.today().strftime('%Y-%m-%d')})."
        )
